...
import {setItemHat} from "../../store/actions/inventoryActions"
import {itemsHat} from "../class/Items"

class InventoryItemHat extends Component {
state={
    itemName: null,
    uid: this.props.auth
}
...
handleMouseOut=(e)=>{
    this.setState({
        itemName: this.props.userCurrent.itemHat
    })
}
render(){

const {userCurrent} = this.props
const items = itemsHat
const foundItemsList = this.props.foundItems && this.props.foundItems
const firstitem = userCurrent.itemHat?(userCurrent.itemHat) :("redcap-hat-idle");
return(
<div className="card z-depth-1 card-border">
<div className="card-content">
<span className="card-title">Hat</span>
<Spritesheet
...
if (itemPart === "hat" && foundItemsList.includes(item.name)){
    let selectIcon = item.name === userCurrent.itemHat?(<div className="selected-inventory-item"></div>):
...
const mapDispatchToProps = (dispatch) =>{
    return{
        selectItem: (data) => dispatch(setItemHat(data))
    }
}
